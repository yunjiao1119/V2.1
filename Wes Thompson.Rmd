...
```{r}
rm(list = ls())
nda18 <- readRDS("E:/ABCD study/R/Outputs/nda18_b1_voi.Rds")

```


```{r}
# imputation didn't produce exact same value for variables with different levels, e.g. household income
head(nda18 %>% select(contains('household.income')), 20)
```
...
ind_demog = c(
  ...
  which(names(nda18) == "race.ethnicity.5level"),
  which(names(nda18) == "race.eth.8level"),
  ...
  which(names(nda18) == "household.income.3level"),
  which(names(nda18) == "household.income.4level"),
...
names(nda18)[which(names(nda18) == "household.income.4level")] = "Household Income"
